...
9. **Dependencies**: DAG edges (Produces, Consumes, Validates, etc).
10. **Artifact Classification**: Layer, Domain, Type.
11. **Required Sections**: The specific markdown headers required in the derived artifact.
12. **Validation Overrides**: Specific validation tests beyond the generic checks.
...
16. **AI Context Overrides**: Priority, required, optional, and forbidden context rules.
17. **Extension Points**: Domain-specific injection points.
18. **Success Criteria**: Business metrics.
19. **AI Contract Overrides**: Artifact-specific generation, validation, and review instructions.

## 4. Generic Validation Contract
Unless overridden, all artifacts MUST comply with:
- **Structural**: Exact match of the standard's `Required Sections`.
- **Semantic**: Zero undefined jargon or hallucinated concepts.
- **Relationship**: Must trace directly to upstream and downstream nodes.
- **Traceability**: No orphaned artifacts allowed.

## 5. Generic Exit Criteria
...
## 6. Generic AI Contract
Unless overridden, AI Swarms interacting with Product Standards MUST obey:
- **AI Generation**: Query stakeholders recursively until all template sections are filled. Do not hallucinate data.
- **AI Validation**: Reject artifacts that leak structural architecture or technical UI implementations unless explicitly allowed by the artifact's domain (e.g., a Design Standard).
- **AI Review**: Flag semantic contradictions between different sections.
- **AI Consumption**: Preserve the core intent of the upstream artifact; do not extrapolate scope.
- **AI Constraints**: Never bypass a Quality Gate. Never break traceability chains.
