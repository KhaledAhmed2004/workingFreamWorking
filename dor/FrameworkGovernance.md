...
This document establishes the meta-rules for managing the Product Discovery Framework. These rules apply universally to all phases of discovery.

## 1. Review Workflow
Before any artifact can be considered complete, it must undergo a rigorous, independent review process.
*   **Self-Correction:** The authoring agent must verify the artifact against the exact prompt criteria.
*   **Peer Review:** The artifact must be reviewed by a simulated independent board (e.g., Enterprise Architecture Review Board) that did not generate the original content.
*   **Rule:** No artifact moves to "Approved" status without a written review and resolution plan.

## 2. Approval Workflow
*   **Resolution Verification:** All mandatory fixes identified in the Review Workflow must be executed.
*   **Executive Approval:** A final executive assessment must confirm that the artifact materially reduces business, technical, or regulatory risk.
*   **Status Change:** Once approved, the artifact is marked as `APPROVED` and added to the official sequence.

## 3. Freeze Workflow
*   **Phase Gates:** A Freeze can only occur at the end of a predefined Phase (e.g., Phase 1: Product Foundation).
*   **Immutability:** Once a Freeze document (e.g., `Sprint1_Freeze.md`) is generated, all artifacts within that Phase become IMMUTABLE.
*   **Unfreezing:** A frozen phase can only be reopened if a critical regulatory, clinical safety, or existential business risk is discovered in a later phase. It cannot be reopened for feature additions or cosmetic changes.

## 4. Versioning Rules
*   **Minor Versions (1.1, 1.2):** Used for intra-sprint iterations and resolution plan executions before a freeze.
*   **Major Versions (1.0, 2.0):** Established only at a Freeze Gate. Sprint 1 Freeze creates Version 1.0 of the Product Foundation. 

## 5. Change Management
*   **The Appendix Rule:** Every approved artifact MUST have the original generation prompt appended to its footer. This ensures total transparency of how the strategy was derived.
*   **Context Continuity:** Future prompts MUST contain the complete text of all previously approved artifacts to ensure the LLM maintains compounding context.

## 6. Traceability Requirements
Every new artifact must explicitly support the artifacts that precede it. 
*   **Example:** A Persona (Phase 3) must map back to a Stakeholder (Phase 2), which must map back to a Goal (Phase 1), which must solve a Problem (Phase 1). Orphaned concepts are strictly prohibited and must be pruned during review.
